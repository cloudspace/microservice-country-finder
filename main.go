...
func main() {
	...
	//TODO connect to local postgres database with the correct credentials
	db_host, db_port, db_user, db_password, db_name := os.Args[1], os.Args[2], os.Args[3], os.Args[4], os.Args[5]
	...
	//NOTE, the input is (Longitude, Latitude)
	query := fmt.Sprintf("SELECT * FROM country WHERE ST_Contains(geom, ST_Transform(ST_GeometryFromText('POINT(%s %s)',4326), 26918))", long, lat);
